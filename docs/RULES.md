...
## Мультидокумент YAML (6.7)

Файлы с несколькими документами (`---`-разделители) валидируются по документам. Ошибки содержат индекс документа (`file#doc2`). JsonSchema и K8sSchema применяются к первому документу.
